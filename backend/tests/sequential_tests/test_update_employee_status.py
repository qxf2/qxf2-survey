...
TEST_DATA = [
    (   # Data to set employee status to active
        {
            "email": {"email": "user_status_to_be_updated@qxf2.com"},
            "status": {"employee_status": "Y"},
        },
    ...
    (   # Data to set employee status of an user that does not exist in database
        {
            "email": {"email": "user_that_does_not_exist@qxf2.com"},
            "status": {"employee_status": "N"},
        },
...
@pytest.mark.parametrize(
    "update_status_data, expected_response, response_msg, user_exists", TEST_DATA
)
def test_update_employee_status(
    update_status_data, expected_response, response_msg, user_exists
):
    ...
    if user_exists:
        ...
        response_diff = [
            i
            for i in employee_details + expected_response
            if i not in employee_details or i not in expected_response
        ]
        response_result = len(response_diff) == 0
        assert len(employee_details) == len(expected_response)
        assert (
            response_result is True
        ), f"There are {len(response_diff)} differences:\n{response_diff}"
    else:
...
def get_employee_details(update_status_data):
    ...
    employee_details = requests.post(
        EMPLOYEE_INFO_URL,
